    ...
    if (sessionTicket != null) {
      ...
      if (ticketChanged) {
        logger.warn(
          ...
          ", sessionTicket: " +
          sessionTicket
        ); // normal scenario but want to log it
      ...
      if (e.getCause() instanceof IOException) {
        ...
        if (
          ...
          throw new BadCredentialsException(
            "Possible error with auth system or infra.. check: 1) configs, urls, ports, 2) caller ticket not expired, 3) cas logs for req ticket: " +
