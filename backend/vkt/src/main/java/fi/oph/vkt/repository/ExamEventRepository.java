...
public interface ExamEventRepository extends BaseRepository<ExamEvent> {
  @Query(
    ...
    " FROM ExamEvent e" +
    " LEFT JOIN e.enrollments en ON en.status = 'COMPLETED' OR en.status = 'AWAITING_PAYMENT' OR en.status = 'AWAITING_APPROVAL' OR en.status = 'EXPECTING_PAYMENT_UNFINISHED_ENROLLMENT' OR en.status = 'CANCELED_UNFINISHED_ENROLLMENT'" +
    " WHERE e.level = ?1" +
