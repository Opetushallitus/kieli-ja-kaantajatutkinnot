...
class PublicRegistrationPage {
  ...
  expectReservationTimerText(visible: boolean, text?: string | RegExp) {
    if (visible) {
      const timer = cy
        .findByTestId('public-registration__reservation-timer-text')
        .should('be.visible');
      if (text instanceof RegExp) timer.invoke('text').should('match', text);
      else if (text) timer.should('have.text', text);
    } else {
