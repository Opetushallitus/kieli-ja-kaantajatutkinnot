...
MSW does not intercept document navigation. In local mock mode, clicking **Maksa tutkintomaksu** fetches the mock payment URL and follows its returned `redirect_url`. The default outcome is a successful payment: the mock stores PAID/COMPLETED and the Done page reads that state again. A mock-only click handler is installed after the local worker starts; Cypress installs the same handler and tests the actual payment-link click, including reload after success. Real payment URLs continue to use normal document navigation.

Authentication mocks still use `fetch` in Cypress followed by a simulated provider redirect. These checks exercise mock session/payment updates and return-page behavior, not live external providers. The default MSW init returns an already authenticated session, so the ordinary local prototype uses **Jatka ilmoittautumiseen**.
