...
In local MSW mode, clicking the payment link completes a successful mock payment and opens Done. The mock-only navigation handler fetches the payment endpoint before following its redirect, so the link does not reach the development proxy. The mock payment link accepts `outcome=pending`, `outcome=cancelled` or `outcome=paid` (default). This is only a test-provider input: the application still uses the subsequent details GET to determine the page. Successful mock submit and cancel requests can be replayed without another transition. These are proposed recovery semantics to discuss with the backend developer, not proof of real transaction safety.
